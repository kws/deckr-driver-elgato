    ...
    async def _presence_loop(self) -> None:
        ...
        while True:
            try:
                ...
                logger.warning(
                    "Elgato manager current state is unavailable; heartbeat will retry",
                    exc_info=True,
                )
    ...
    async def _withdraw_presence(self) -> None:
        ...
        with anyio.CancelScope(shield=True):
            try:
                ...
                logger.debug("Elgato manager presence changed before withdrawal")
            except StateUnavailable:
                logger.warning("Failed to withdraw Elgato manager presence", exc_info=True)
            ...
            HardwareInventory(
                ...
                devices={
                    device_id: HardwareInventoryDevice(
                        deviceId=device_id,
                        hardwareType=device.name or "elgato",
